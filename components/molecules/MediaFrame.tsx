...
export function MediaFrame({ src, alt, variant = 'standard', priority = false, sx, ...props }: MediaFrameProps) {
  return (
    <Box sx={{ position: 'relative', aspectRatio: ratios[variant], overflow: 'hidden', borderRadius: 2, ...sx }} {...props}>
      <Image src={src} alt={alt} fill priority={priority} sizes="(max-width: 900px) 100vw, 50vw" style={{ objectFit: 'cover' }} />
    </Box>
  );
}
